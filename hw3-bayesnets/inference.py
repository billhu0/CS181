...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables: List[str], evidenceDict, eliminationOrder):
        """
        ...
        "*** YOUR CODE HERE ***"
        factors: List[Factor] = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for variable in eliminationOrder:
            factors, joined = joinFactorsByVariable(factors, variable)

            # unless the variable is the only remaining unconditioned variable, we take the result
            if len(joined.unconditionedVariables()) > 1:
                factors.append(eliminate(joined, variable))

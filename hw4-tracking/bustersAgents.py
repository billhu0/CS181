...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        ghost_pos = [max(pos_dist, key=pos_dist.get) for pos_dist in livingGhostPositionDistributions]

        # find the closest ghost
        closest_ghost = sorted(ghost_pos, key=lambda pos: self.distancer.getDistance(pos, pacmanPosition))[0]

        # then choose an action that minimizes the maze distance to the closest ghost
        return sorted(legal, key=lambda a: self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, a), closest_ghost))[0]

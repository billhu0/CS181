...
class Expr:
    ...
    def __init__(self, op, *args):
        ...
        if not args and not is_prop_symbol(self.op):
            raise SyntaxError("Unacceptable symbol base name (%s). Name must start with an upper-case alphabetic "
                              "character that and is not TRUE or FALSE. Furthermore, only the following are allowed: "
                              "capital and lower case alphabetic, 0-9, _, \",\", [, and ]." % self.op)
...
class PropSymbolExpr(Expr):
    ...
    def __init__(self, sym_str: str, *index: Tuple[int] or int, time: int = None):
        ...
        if not is_prop_symbol(sym_str):
            raise SyntaxError(
                "Unacceptable symbol base name (%s). Name must start with an upper-case alphabetic character that and "
                "is not TRUE or FALSE. Furthermore, only the following are allowed: capital and lower case "
                "alphabetic, 0-9, _, \",\", [, and ]." % sym_str)
        ...
        if len(index) > 0:
            if len(index) > 4:
                raise SyntaxError(
                    "Too many arguments to SymbolExpr constructor. SymbolExpr(symbol_str, [index1], [index2], "
                    "[index3], [index4], time=[time]), or fewer indicies -- possibly 0.")
            if len(index) == 1:

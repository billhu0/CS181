...
import functools
import util
import hunters  # for typing alias
import bayesNet  # for typing alias
from typing import List, Tuple, Dict  # for typing alias
...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict: Dict[str, List[str] or List[List[str]]] = {}

    "*** YOUR CODE HERE ***"
    # What is 'gameState'?
    # gameState.getPossibleHouses() returns [(3,3), (3,7), (11,3), (11,7)]
    # gameState.getHouseWalls(housePos) returns [(4,4), (2,3), ...] where housePos is (3,3)
    # OBS_VAR_TEMPLATE is a string 'obs(%d,%d)'
    # We simply replace the two '%d' with each wall of each house position

    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVars.append(OBS_VAR_TEMPLATE % obsPos)

    for pos in [X_POS_VAR, Y_POS_VAR]:  # X_POS_VAR = 'xPos', Y_POS_VAR = 'yPos'
        for house in HOUSE_VARS:  # HOUSE_VARS = ['foodHouse', 'ghostHouse']
            edges.append((pos, house))

    for house in HOUSE_VARS:  # HOUSE_VARS = ['foodHouse', 'ghostHouse']
        for obs in obsVars:  # obsVars = ['obs(1,2)', ...]
            edges.append((house, obs))

    # # variableDomainsDict =
    # # {'xPos': ['foodLeft', 'ghostLeft'], 'yPos': ['bothTop', 'bothBottom', 'leftTop', 'leftBottom']}
...
def fillYCPT(bayesNet: bayesNet.BayesNet, gameState: hunters.GameState):
    """
    ...
    yFactor: bayesNet.Factor = bn.Factor([Y_POS_VAR], [], bayesNet.variableDomainsDict())
    "*** YOUR CODE HERE ***"
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    """ *** YOUR CODE HERE *** """
    factor: bayesNet.Factor = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)

    pairs: List[Tuple[Dict, float]] = []

    # traverse each possible assignment and get probability
    for assignment in factor.getAllPossibleAssignmentDicts():
        pairs.append((assignment, factor.getProbability(assignment)))

    # sort by probability
    pairs.sort(key=lambda x: x[1])

    # return the position with highest probability
    return pairs[-1][0]
    """ *** END YOUR CODE HERE *** """
